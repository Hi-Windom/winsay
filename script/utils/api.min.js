 ...
 */
export function addinsertCreateElement(
  fatherElement,
  addElementTxt,
  setId = null
) {
...
export async function getConfigesStore(j) {
  let url = "http://127.0.0.1:6806/api/sillot/getConfigesStore";
  return parseRes(request(url, j));
}
...
export const getNewValueFromDomByID = async (id) => {
...
export async function initAllPropFromIDBAsync(dom) {
  ...
  const _checkedInit = async (obj) => {
    ...
    await _bind(obj.id);
    return localforage.getItem(obj.id).then(async (v) => {
      if (!isEmptyString(v)) {
        obj.bindIDB = "true";
      } else {
        obj.bindIDB = "false";
  ...
  const _propInit = async (id) => {
    ...
    await _bind(id);
    return localforage.getItem(id).then(async (v) => {
  ...
  let normalInputList = dom.querySelectorAll(
    "input[id^='SC_winsay_cp']:not([type='checkbox'])"
  );
  let checkboxList = dom.querySelectorAll(
    "input[type='checkbox']:not([id^='NoSync'])"
  );
  ...
  window.sofill.cp.awaitInitItem = childsLength;
  await _forEach(selectList, async (dom) => {
    await _propInit(dom.id);
    window.sofill.cp.awaitInitItem -= 1;
    window.sofill.cp.inited += 1;
  });
